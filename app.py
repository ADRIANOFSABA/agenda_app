...
def criar_admin_inicial():
    ...
    if qtd == 0:
        c.execute("INSERT INTO usuarios (empresa_id, nome, email, senha_hash, perfil, ativo) VALUES (?, ?, ?, ?, ?, ?)", (None, "Administrador Master", "admin@agendapro.com", hash_senha("123456"), "master", 1))
        conn.commit()
criar_admin_inicial()
...
def autenticar(email: str, senha: str):
    ...
    row = c.execute("SELECT u.*, e.nome_fantasia as empresa_nome FROM usuarios u LEFT JOIN empresas e ON e.id = u.empresa_id WHERE lower(u.email) = ? AND u.ativo = 1", (email,)).fetchone()
    if not row:
        return None
...
def registrar_movimentacao_financeira(data, tipo, categoria, descricao, valor, forma_pagamento, conta_nome, origem, agendamento_id=None):
    executar("INSERT INTO movimentacoes_financeiras (empresa_id, data, tipo, categoria, descricao, valor, forma_pagamento, conta_nome, origem, agendamento_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (precisa_empresa_id(), str(data), tipo, categoria, descricao, float(valor), forma_pagamento, conta_nome, origem, agendamento_id))
...
def registrar_movimentacao_estoque(data, produto_id, produto_nome, tipo_movimentacao, quantidade, valor_unitario, origem, observacao):
    executar("INSERT INTO estoque_movimentacoes (empresa_id, data, produto_id, produto_nome, tipo_movimentacao, quantidade, valor_unitario, origem, observacao) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (precisa_empresa_id(), str(data), int(produto_id), produto_nome, tipo_movimentacao, float(quantidade), float(valor_unitario), origem, observacao))
...
def cadastrar_produto_automatico(nome, unidade, custo_unitario, origem_obs="Importado por XML"):
    executar("INSERT INTO produtos (empresa_id, nome, tipo, unidade, estoque_atual, estoque_minimo, custo_unitario, preco_venda, observacao) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (precisa_empresa_id(), nome.strip(), "Mercadoria", unidade.strip(), 0.0, 0.0, float(custo_unitario), 0.0, origem_obs))
    return c.lastrowid
...
def render_agenda_cards(df):
    ...
    for _, row in df.iterrows():
        ...
        st.markdown(f"<div class='schedule-premium'><div class='time'>{str(row['hora'])[:5]}</div><div class='client'>{row['cliente']}</div><div class='meta'>{row['profissional']} • {row['servico'] if row['servico'] else 'Sem serviço'}</div><div class='meta'>Total: {moeda_br(total)} • Duração: {dur} min</div><div class='meta'>Pagamento: {row['forma_pagamento'] if pd.notna(row['forma_pagamento']) and row['forma_pagamento'] else '-'}</div>{badge_status(row['status'])}</div>", unsafe_allow_html=True)
        ...
        with tabs[0]:
            ...
            if st.button("Salvar empresa"):
                if nome_fantasia.strip():
                    executar("INSERT INTO empresas (nome_fantasia, razao_social, documento, telefone, email, pix_chave, pix_beneficiario, pix_cidade, ativa) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (nome_fantasia.strip(), razao_social.strip(), documento.strip(), telefone.strip(), email_emp.strip().lower(), pix_chave.strip(), pix_beneficiario.strip(), pix_cidade.strip(), int(ativa)))
...
if menu == "🛎️ Recepção":
    ...
    with col_r1: data_recepcao = st.date_input("Data da recepção", value=datetime.now().date(), key="rec_data")
    with col_r2: termo_recepcao = st.text_input("Buscar cliente / profissional / serviço", key="rec_busca", placeholder="Filtro automático")
    with col_r3: status_recepcao = st.selectbox("Status", ["Todos","Agendado","Confirmado","Em atendimento","Concluído","Cancelado"], key="rec_status")
    ...
    with h1: dt_ini = st.date_input("De", value=datetime.now().date(), key="hist_ini")
    with h2: dt_fim = st.date_input("Até", value=datetime.now().date(), key="hist_fim")
    with h3: termo_hist = st.text_input("Buscar no histórico", key="hist_busca", placeholder="Cliente, profissional, serviço...")
...
if menu == "🗓️ Agendamentos":
    ...
    with aba2:
        ...
        with col_f1: termo = caixa_busca("Pesquisar agendamento", "busca_agendamento_modulo")
        with col_f2: filtro_status = st.selectbox("Filtrar status", ["Todos","Agendado","Confirmado","Em atendimento","Concluído","Cancelado"], key="agendamento_filtro_status")
        with col_f3: filtro_data = st.text_input("Filtrar data", key="agendamento_filtro_data", placeholder="AAAA-MM-DD")
        ...
        if df_ag.empty:
            ...
            df_servicos = consultar_df(f"SELECT nome FROM servicos WHERE {cond} ORDER BY nome", params)
            df_contas = consultar_df(f"SELECT nome FROM contas_financeiras WHERE {cond} ORDER BY nome", params)
            clientes = df_clientes["nome"].tolist(); profissionais = df_profissionais["nome"].tolist(); servicos_cadastrados = df_servicos["nome"].tolist(); contas = df_contas["nome"].tolist()
...
if menu == "📦 Produtos e Estoque":
    ...
    with aba1:
        ...
        with c3: unidade = st.text_input("Unidade", key="prod_unid")
    ...
    with aba2:
        ...
        if df_prod.empty:
            ...
            with c5: origem = st.text_input("Origem", key="est_origem")
